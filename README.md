...
- v1.3.4 在客户端升级后清理旧网页缓存，连接时重新验证入口页面；不清除登录 Cookie、页面偏好或独立图片缓存。
- 随机动态和图库 API 使用 `no-store`，不改变服务器随机种子、排序、游标及筛选参数。预览图和原图不受此策略影响。
- 全部动态默认随机、背景大图及详情图片手势仍由已部署的 Lumic 网页提供。若升级后仍缺失，请先确认服务器已部署当前 Lumic，并更新 Android System WebView/Chrome。
